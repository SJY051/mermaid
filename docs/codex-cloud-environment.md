...
**setup의 실패가 PR 때문이어서는 안 됩니다.** 캐싱이 꺼져 있으면 Codex는 **PR 커밋을 체크아웃한 뒤에** setup을 돌립니다([문서](https://learn.chatgpt.com/docs/environments/cloud-environment)). 그래서 setup이 실패하면 **리뷰 에이전트가 시작조차 못 합니다** — 리뷰가 가장 필요한 PR(테스트를 깬 PR)에서 정확히 리뷰가 안 붙습니다. (이 함정은 Codex 리뷰어가 PR #3에서 직접 잡았습니다.)

그런데 에이전트 단계엔 인터넷이 없으므로, 테스트 **런타임** 의존성(H2·MariaDB 드라이버 등 `testRuntimeClasspath`)까지 여기서 받아둬야 합니다. `testClasses`나 `compileTestJava`로는 부족합니다 — 그건 컴파일 의존성만 받고, 에이전트가 오프라인에서 `test`를 돌릴 때 `No cached version ... for offline mode`로 죽습니다(실측).

해법은 **setup에서 `test`를 한 번 돌리되 실패를 삼키는 것**입니다. 테스트 실행이 모든 런타임 JAR을 캐시에 넣고, `|| true`가 PR의 테스트 실패로부터 setup을 지킵니다. PR이 테스트를 깨도 setup은 exit 0으로 계속되고, 에이전트는 오프라인에서 같은 `test`를 다시 돌려 그 실패를 **직접 잡습니다**(실측: 주입한 실패가 `276 tests completed, 1 failed`로 잡힘).
...
# 컴파일 의존성 + 테스트 런타임 의존성(H2·MariaDB 드라이버)을 전부 캐시에 넣는다.
# `|| true`가 핵심이다: PR이 깨뜨린 테스트가 여기서 실패해도 setup은 살아남아야
# 리뷰 에이전트가 뜬다. 그 뒤 에이전트가 오프라인 `test`로 실패를 실제로 잡는다.
