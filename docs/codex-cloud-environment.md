...
**setup은 하이드레이트만 하고, PR 내용으로는 절대 실패하지 않아야 합니다.** 캐싱이 꺼져 있으면 Codex는 **PR 커밋을 체크아웃한 뒤에** setup을 돌립니다([문서](https://learn.chatgpt.com/docs/environments/cloud-environment)). 그래서 setup이 실패하면 **리뷰 에이전트가 시작조차 못 합니다** — 리뷰가 가장 필요한 PR에서 정확히 리뷰가 안 붙습니다. 검증(테스트가 통과하는가, lockfile이 맞는가)은 **CI와 에이전트의 몫**이지 setup의 몫이 아닙니다. (이 두 함정 모두 Codex 리뷰어가 PR #3에서 직접 잡았습니다 — 한 번은 백엔드 줄, 한 번은 프론트 줄.)

두 줄 다 PR 의존적이라 `|| true`로 감쌉니다.

- **프론트: `--frozen-lockfile`을 빼세요.** 그건 검증(lockfile이 package.json과 맞는지)이고, lockfile을 갱신 안 한 PR에서 exit 1로 죽어 setup을 멈춥니다(실측). plain `pnpm install`은 드리프트에도 node_modules를 채워 에이전트가 테스트를 돌릴 수 있게 합니다. **드리프트 자체는 CI가 `--frozen-lockfile`로 잡습니다**(`.github/workflows/ci.yml`) — Codex setup까지 이중으로 검증할 필요가 없습니다.
- **백엔드: `test`를 한 번 돌리되 삼킵니다.** 에이전트 단계엔 인터넷이 없으므로, 테스트 **런타임** 의존성(H2·MariaDB 드라이버 등 `testRuntimeClasspath`)을 여기서 받아둬야 합니다. `testClasses`로는 부족합니다 — 컴파일 의존성만 받아 오프라인 `test`가 `No cached version`으로 죽습니다(실측). `test`를 실행하면 모든 런타임 JAR이 캐시에 들어오고, `|| true`가 PR의 실패로부터 setup을 지킵니다. 에이전트는 같은 `test`를 오프라인으로 다시 돌려 실패를 **직접 잡습니다**(실측: 주입한 실패가 `276 tests completed, 1 failed`로 잡힘).
...
# 하이드레이트만. --frozen-lockfile은 검증이라 뺀다(드리프트 PR에서 setup을 죽인다).
# lockfile 드리프트는 CI가 --frozen-lockfile로 잡는다. `prepare`가 pre-commit 가드도 건다.
(cd frontend && pnpm install) || true

# 컴파일 + 테스트 런타임 의존성(H2·MariaDB 드라이버)을 전부 캐시에 넣는다. `|| true`가 핵심:
# PR이 깨뜨린 테스트가 여기서 실패해도 setup은 살아 리뷰 에이전트가 뜬다. 에이전트가
# 오프라인 `test`로 그 실패를 실제로 잡는다.
(cd backend && ./gradlew test --no-daemon) || true
```
...
maintenance 스크립트는 **캐시된 컨테이너를 이어 쓸 때만** 실행됩니다. 우리는 캐싱을 껐으므로 매번 새 컨테이너가 뜨고, 이 스크립트는 한 번도 돌지 않습니다. 채워두면 검증되지 않은 코드가 캐싱을 켜는 날 조용히 실행되기 시작합니다.

캐싱을 켜기로 한다면 그때 함께 작성하고, **그때 `pnpm install`이 새 패키지를 받을 인터넷이 있는지부터 확인하세요.**
...
- **setup 스크립트 순서를 그대로 재현:** 콜드 `GRADLE_USER_HOME`에서 `test || true` 실행 → 이후 `--offline test`가 캐시만으로 성공. `testClasses`만으로는 `testRuntimeClasspath`(H2·MariaDB)가 안 받아져 오프라인에서 `No cached version`으로 실패하는 것을 확인.
- **PR이 테스트를 깬 경우:** 테스트에 실패를 주입한 뒤, setup의 `test`는 실패하지만 `|| true`로 exit 0 → 오프라인 `test`가 `276 tests completed, 1 failed`로 그 실패를 **직접 감지**. setup은 살고 에이전트가 결함을 잡는 두 조건을 동시에 확인.
- **lockfile 드리프트 PR:** package.json에 lockfile 없는 의존성을 추가한 뒤, `pnpm install --frozen-lockfile`은 **exit 1로 죽지만**(= setup 중단), plain `pnpm install`은 **exit 0으로 node_modules를 채우는 것**(새 의존성 포함)을 확인. 그래서 setup은 후자를, CI는 전자를 씁니다.

## 아직 모르는 것

- **maintenance 스크립트에 인터넷이 있는지** 문서가 말하지 않습니다("not stated"). setup 단계는 **항상** 네트워크가 열린다고 설정 화면이 명시하지만, 그 문장은 maintenance를 덮지 않습니다. 캐싱을 껐으므로 maintenance는 실행되지 않아 지금은 무해합니다 — **캐싱을 켜는 날 가장 먼저 확인할 항목입니다.**
