...
**Goals**
- 로그인 없이 즉시 대화 시작
- LLM의 검색 질의 출력은 형태를 강제하고, 공식 레코드가 있으면 **서버가 최종 카드를 작성·교차검증**한 뒤에만 화면에 보냅니다
...
│  ServerAuthoredAnswerBuilder  공식 레코드 → canonical card │
...
### 2-2. "네이티브 서치 그라운딩 강제 활성화" → 공공 API 조회 + 단계적 서버 작성 응답
...
**교정:** 공공 API 조회 경계를 유지하고, 응답 작성은 단계적으로 서버 소유로 전환합니다. 초기 2-pass 구현은 2026-07-10 완료됐고(DEV-403), non-empty 응답은 2026-07-16 승인에 따라 서버 작성으로 변경됐습니다.
...
**추출기의 출력은 사실이 아니라 질의입니다.** 모델이 `"Acetaminophen"`을 제안하면, 그런 제품이 존재하는지·무엇이 들었는지·무슨 효능으로 허가됐는지는 **정부 DB가 결정**합니다. 모델이 엉뚱한 걸 제안하면 검색 결과가 비고, non-empty 카드에는 아무 영향도 주지 못합니다. 모델이 약을 존재하게 만들 수는 없습니다 — 서버가 실제 조회 레코드로만 카드를 만듭니다.
...
| **e약은요 항목이 없으면 제외** | 기존 조회 완전성 계약을 유지합니다. 레코드 범위가 불완전한 제품을 canonical card 후보로 올리는 정책 변경은 별도 검토 대상입니다. (`수출용` 4건 전부 e약은요 없음) |
...
non-empty 응답의 `source_refs`, `data_status`, `drugs[].source_ref_id`는 전부 **서버가 직접 씁니다**. 모델은 그 레코드가 라이브 API에서 왔는지 픽스처에서 왔는지 판단하거나 참조를 복사하지 않습니다. builder는 product↔source 일대일 결합과 기존 validator를 모두 통과한 카드만 반환합니다.
...
이 측정에서 공공 조회는 전체의 4%도 아니었고 provider 지연과 편차가 대부분이었습니다. 공식 데이터를 이미 얻은 뒤 whole-answer 포맷 오류로 카드를 폐기하는 비용까지 확인되어, non-empty 응답을 서버 작성으로 전환했습니다.
초기 2-pass 구현은 왕복을 2회로 늘려 provider 노출을 키웠습니다. 그때 정한 운영 값과 현재 의미는 다음과 같습니다.
...
위반 또는 product↔source mapping 불일치 시 카드 전체를 숨기고 고정 safe fallback을 반환합니다(§7).
...
- **FR-03** (Must): 모든 응답은 `MermAidAnswerV1` 형태 + **서버 후처리 불변조건**을 통과한다(§2-15). non-empty drug card는 모델 출력이 아니라 서버 record mapping이다. 지도는 `ui_actions[]`.
...
| `02_ARCHITECTURE` | TAKE WITH EDITS | 어댑터 경계·공공 조회·data mode 채택. 초기 2-pass 응답 작성은 staged server-authored boundary로 개정 |
| `03_API_CONTRACT` | **TAKE WITH EDITS** | `MermAidAnswerV1` 형태와 후처리 불변조건은 유지하되, non-empty card 작성자는 서버로 변경. 프로필 CRUD 엔드포인트 추가 |
