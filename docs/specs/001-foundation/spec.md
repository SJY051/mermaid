...
- 병원은 심평원 **두 서비스를 엮어야** 합니다. 둘은 짝이고, 하나만으로는 아무것도 못 합니다.
  ...
  | 서비스 | 경로 | 역할 | 상태 |
  |---|---|---|---|
  | 병원정보서비스 | `B551182/hospInfoServicev2/getHospBasisList` | `xPos`/`yPos`/`radius`(m)로 검색. **`ykiho`를 발급하는 유일한 곳** | ✅ **200** (2026-07-10 승인) |
  | 의료기관별상세정보서비스 | `B551182/MadmDtlInfoService2.7/getDtlInfo2.7` | `ykiho`로 진료시간 조회. **검색 오퍼레이션이 없음** | ⛔ 403 승인 대기 |

  - **버전 접미사가 두 번 붙습니다**: 서비스 `…2.7`, 오퍼레이션 `getDtlInfo2.7`. `…2.8`도, `…2.7/getDtlInfo`(접미사 없는 오퍼레이션)도 **404 `API not found`**입니다. 우리 `application.yml`에 둘 다 틀리게 적혀 있었습니다 (2026-07-10 교정).
  - **목록은 실측했습니다** (`fixtures/hospital_list.json`). 실제 필드: `ykiho`, `yadmNm`, `addr`, `telno`, `XPos`(경도)/`YPos`(위도), `distance`, `clCd`/`clCdNm`(종별), `sidoCdNm`, `sgguCdNm`, `postNo`, `estbDd`, 의사 수 계열.
    - **`distance`는 미터입니다.** 약국 API는 km입니다 — **반대입니다.** Haversine으로 대조해 확인(오차 0.6% 이내). 39자리 문자열로 옵니다.
    - **`radius`는 필수입니다.** 빼면 좌표를 줘도 전국 79,727건이 옵니다.
    - **진료시간 필드는 목록에 없습니다.** 상세 서비스에 있고, 그건 아직 403입니다.
  - `trmtMonStart`~`trmtSunEnd`, `lunchWeek`, `noTrmtSun`, `emyNgtYn` 같은 필드명은 **상세 서비스 것이고 200 응답을 본 적이 없습니다.** 승인 전까지 파서를 쓰지 마세요. N+1이므로 캐싱은 필수입니다.
  - **[미해결] 상세 서비스가 403.** 우리 키로 403, 아무 문자열이나 넣으면 401 — 게이트웨이가 둘을 구분하므로 **키는 유효하고 활용신청 승인이 안 난 것**입니다. 국립중앙의료원의 병원(`HsptlAsembySearchService`)·응급실(`ErmctInfoInqireService`) 서비스도 403이고, 약국 서비스(`ErmctInsttInfoInqireService`)만 200입니다 → **승인은 기관이 아니라 서비스 단위**입니다.

  > **그래서 지금 병원은:** 위치·이름·전화·종별·거리까지는 보여줄 수 있고, **영업 여부는 알 수 없습니다.**
  > `isOpenNow`는 `null`(모름)이어야 합니다. `false`(닫힘)로 단정하면 §2-13에서 고친 그 버그를 되풀이하는 것입니다.
...
> **알려진 한계 — 약물 계열 교차반응 (2026-07-10 실측에서 드러남).**
...
>
> 계열 교차반응 표는 **임상 지식**이며, 우리가 지어내서는 안 됩니다. 두 가지 중 하나로 가야 합니다.
> ① 검토자가 서명한 계열 표를 `ingredients/`에 추가하고 `warning`(차단 아님)을 발생시킨다.
> ② 계열 판정을 하지 않는다고 명시하고, 알레르기가 선언된 턴에서는 **같은 적응증의 다른 계열을 우선**한다.
>
> **어느 쪽도 지금 코드에 없습니다.** PM/QA 레인 + 임상 자문이 필요합니다.
...
| 심평원 병원정보 | **HTTP 403 Forbidden** (2026-07-10 기준) | **활용신청 승인 필요.** ASQi 확인 |
...
- ~~심평원 상세정보 서비스 버전 접미사 (`2.7` vs `2.8`)~~ → **해결: `MadmDtlInfoService2.7/getDtlInfo2.7`** (2026-07-10 실측)
- 심평원 두 서비스의 실제 응답 필드 → **승인 전까지 확인 불가.** 문서만 보고 파서를 쓰지 말 것
