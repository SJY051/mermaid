...
| 콜드 캐시 전체 왕복 | 112초 (식약처 호출 약 29회 포함) |

**이건 우리 코드가 느린 게 아니라 프로바이더가 느린 것입니다.** 다만 우리가 왕복을 2회로 늘려 노출을 두 배로 만들었습니다. 그래서:
...
**남은 일:** 콜드 112초는 시연에 부담스럽습니다. `DrugService.retrieve()`의 상세 조회 3건이 순차이고 각각 DUR을 4회 부릅니다(총 약 29회). 병렬화가 첫 번째 개선입니다. FE는 **로딩 상태를 반드시 그려야 합니다.**
...
- 병원은 두 서비스를 엮습니다: `hospInfoServicev2/getHospBasisList`(`xPos`/`yPos`/`radius`(m) 있음) → `ykiho` → `MadmDtlInfoService2.8/getDtlInfo`(`trmtMonStart`~`trmtSunEnd`, `lunchWeek`, `noTrmtSun`, `emyNgtYn`). **N+1이므로 캐싱 필수.**
  - **[미해결] 심평원 병원정보 API가 HTTP 403 Forbidden을 반환합니다.** 활용신청 승인이 필요합니다.
...
| `response_format` | **모델마다 다릅니다.** `glm-5.2`·`kimi-k2.6`은 `json_schema` strict를 정확히 지킴(`oneOf`/`$defs`/`const`/`format`까지). `deepseek-v4-*`·`qwen3.7-max`는 **HTTP 400**. `minimax-m3`는 200이지만 `<think>` 추론을 평문으로 반환 |
...
- **AR-02 — `synonyms.tsv`의 검토자 칸이 전부 `TODO`인 채로 차단이 일어난다.**
  - **위험:** §2-12는 "**검증된** 동의어"만 `blocked`를 만들 수 있다고 규정하지만, 현재 7행 전부 미서명이다. 특히 `dexibuprofen → ibuprofen` 행의 근거란은 문자 그대로 *"cross-reactivity is expected"* — **서명 없는 교차반응 근거가 이미 차단을 만들고 있다.**
  - **방향성:** 차단은 과잉이 안전한 방향(약을 못 먹는 밤 하나)이므로 즉시 위험은 아니다. 그래도 규정 위반이다.
  - **조치:** DEV-305에서 PM/QA가 7행에 서명한다. 서명 전까지 행을 **추가하지 않는다.**
