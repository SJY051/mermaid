...
- 병원은 심평원 **두 서비스를 엮어야** 합니다. 둘은 짝이고, 하나만으로는 아무것도 못 합니다.
  ...
  | 의료기관별상세정보서비스 | `B551182/MadmDtlInfoService2.7/getDtlInfo2.7` | `ykiho`로 진료시간 조회. **검색 오퍼레이션이 없음** |
  ...
  - **[미해결] 두 서비스 모두 HTTP 403.** 우리 키로 403, 아무 문자열이나 넣으면 401 — 게이트웨이가 둘을 구분하므로 **키는 유효하고 활용신청 승인이 안 난 것**입니다. 같은 판정법으로 국립중앙의료원의 병원(`HsptlAsembySearchService`)·응급실(`ErmctInfoInqireService`) 서비스도 403이고, 이미 쓰고 있는 약국 서비스(`ErmctInsttInfoInqireService`)만 200입니다 → **승인은 기관이 아니라 서비스 단위**입니다.
  - `trmtMonStart`~`trmtSunEnd`, `lunchWeek`, `noTrmtSun`, `emyNgtYn` 같은 필드명은 **문서에서 옮겨 적은 것이고 200 응답을 본 적이 없습니다.** 승인 전까지 파서를 쓰지 마세요 (§3의 "추측한 사실은 실측이 뒤집는다"). N+1이므로 캐싱은 필수입니다.
