...
**함정 셋:**
1. 성분명은 **영문이고 대소문자를 가립니다.** `Acetaminophen` → 1,357건, `acetaminophen` → **0건**, `아세트아미노펜` → **0건**.
2. 상세 오퍼레이션은 **`…Dtl Inq06`**입니다(서비스는 07). `getDrugPrdtPrmsnDtlInq07`은 **404**.
...
**사실 (2026-07-10 실제 호출로 확인, `backend/src/test/resources/fixtures/` 참조):**
...
> 실제 응답은 `backend/src/test/resources/fixtures/`에 있습니다. **그것으로 개발하세요.**
