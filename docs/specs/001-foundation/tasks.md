...
1. **DEV-203 병원 검색.** **병원정보서비스(목록)는 승인되어 200입니다.** 실제 응답을 `fixtures/hospital_list.json`에 박아뒀고, 함정은 `fixtures/README.md` 12~15번에 있습니다 — 특히 **`distance`가 미터**(약국은 km!), **`radius` 필수**, **`XPos`=경도/`YPos`=위도**.
   **의료기관별상세정보서비스(진료시간)는 아직 403**이라, 병원의 `isOpenNow`는 `null`이어야 합니다. `false`로 쓰지 마세요.
