...
| OUT-03 | P0 → **P1, partly closed** | For a legitimately retrieved product, rewrite ingredients, warnings, directions, prescription status, or server-computed `allergyCheck`; for example downgrade `blocked` to `no_match_found`. | The context contains the server result at `DrugContextRetriever.java:102-147`, but `ground()` preserves model drug cards at `ChatProxyController.java:195-209`; `AnswerValidator.java:44-60,88-95` checks shape/name/source sets rather than equality to the retrieved record. | **Closed, field by field:** `allergyCheck` — the server stamps its own verdict over the model's (`groundAllergyChecks`, PR #60). Ingredients — set equality against the retrieved record (`INV6_INGREDIENT_MISMATCH`). **Directions — post-processing invariant 7 (PR #77): the server writes this field with 식약처's own 용법용량, verbatim; the model's dosing text is discarded unread.** (A first attempt only *checked* the model's numbers against the label. Review broke it in one line: 만 12세 이상 contains "12", so "Take 12 tablets once daily" passed — the digit was there, as an AGE. Numbers have roles and a substring does not carry one. Removing the model's authority beats verifying its output.) <br>**Still open — do not read this row as done:** `warnings[]` and `prescriptionStatus` are still model-authored and rendered. The model is told to copy every `durWarnings` entry; nothing checks that it did, so a dropped contraindication or an invented warning still reaches the card. That is the remaining OUT-03 surface, and it is the same shape as the one just closed: derive it from the server record, do not ask the model to be faithful. |
...
- OUT-02 and OUT-03 need an architectural grounding decision, not a quick regex. OUT-04 through OUT-09 overlap validator-contract work owned by DEV-601, so this branch reports them without editing `AnswerValidator` or its owned tests/fixtures.
  - **Update 2026-07-14 (PR #77).** OUT-03's dosing half took that decision rather than a regex on prose, and it took it twice. The first answer *verified* the model's numbers against the label and was defeated by a number in the wrong role (an age read as a tablet count). The second removed the authority instead: the server writes the dose, verbatim from 식약처, and the model's version is never read. Nothing to verify, because nothing was authored. That is the general shape for the rest of OUT-03 — **bind the model to the server's record on the parts a person ACTS on, and leave it free on the parts they only read.** Where you cannot check an output cheaply and soundly, take away the authority to produce it.
