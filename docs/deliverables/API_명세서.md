...
- **약국 (`facility:nmc:<hpid>`) → `200`.** `getParmacyBassInfoInqire` 한 번으로 복원한 `Facility`를 돌려줍니다. **이름·주소·좌표는 필수** — 이 셋 중 하나라도 없는 행은 `200`이 아니라 `404`입니다. 전화·주간 시간표는 제공되면 포함(없으면 `null`/`unknown`). 단건 조회에는 기준 좌표가 없으므로 **`distanceMeters`는 `null`**입니다 (거짓 `0`이 아니라). `FE`는 이 필드를 `number | null`로 받아야 합니다 (연결은 후속 #92, **미완료**).
