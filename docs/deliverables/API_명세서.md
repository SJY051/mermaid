...
| GET | `/facilities/{id}` | ✅ 약국 `200` / 병원 `501` |
| GET | `/drugs` | ✅ 동작 |
...
| `operation_preference` | enum | | `any` | `any` \| `confirmed_open_only` \| `open_or_unknown` |
| `open_now` | boolean | | — | 하위 호환 전용. `operation_preference`와 동시 지정 불가 |
| `type` | enum | | `pharmacy` | `pharmacy` \| `hospital` \| `emergency_room` |
| `limit` | int | | `50` | 1 ~ 50; 운영 상태 필터·정렬 후 적용 |

- `operation_preference=any`는 거리순입니다. `confirmed_open_only`도 필터 후 거리순입니다.
  `open_or_unknown`은 확인된 영업 중 시설을 먼저, 운영시간 미상 시설을 다음에 두며 각 그룹 안은 거리순입니다.
  정렬 파라미터는 없습니다.
- 항상 요청한 `limit` 이하만 답합니다. `confirmed_open_only`와 `open_or_unknown`은 약국과 병원에서
  거리순 최대 100개 후보의 시간표를 먼저 조회한 뒤 필터·정렬하고 마지막에 `limit`을 적용합니다.
  이 100개는 상세 시간표 API 호출량과 첫 로드 시간을 지키는 안전 상한이므로 반경 전체에 대한 완전 검색은 아닙니다.
- `confirmed_open_only`는 **영업 중임을 아는** 곳만 남깁니다. `open_or_unknown`은 확인된 닫힘만 제외하고
  시간표를 읽지 못한 시설을 보존합니다. `any`는 세 상태를 모두 보존합니다.
- 기존 `open_now=true`는 `confirmed_open_only`, `open_now=false` 또는 생략은 `any`로 해석합니다.
  두 파라미터를 함께 보내면 `400 INVALID_REQUEST`입니다.
...
> `type=emergency_room`은 국립중앙의료원 위치 데이터입니다. 목록은 약 100m 격자 중심에서 조회·캐시하지만, 거리는 현재 요청 좌표에서 m로 다시 계산합니다. 운영시간은 제공되지 않아 `isOpenNow`가 항상 `null`입니다. 따라서 `confirmed_open_only`/`open_now=true`에는 포함되지 않고 `open_or_unknown`에는 포함됩니다. Fixture·hybrid fallback 결과는 `source.dataMode="fixture"`로 명시합니다.

### `GET /facilities/{id}` — 약국 `200` / 병원 `501`

UI-03 상세용. `id`는 네임스페이스 id입니다 (`facility:nmc:<hpid>` 또는 `facility:hira:<base64url(ykiho)>`).

- **약국 (`facility:nmc:<hpid>`) → `200`.** `getParmacyBassInfoInqire` 한 번으로 복원한 `Facility`를 돌려줍니다. **이름·주소·좌표는 필수** — 이 셋 중 하나라도 없는 행은 `200`이 아니라 `404`입니다. 전화·주간 시간표는 제공되면 포함(없으면 `null`/`unknown`). 단건 조회에는 기준 좌표가 없으므로 **`distanceMeters`는 `null`**입니다 (거짓 `0`이 아니라). FE 계약은 이 필드를 `number | null`로 받고 `null`을 `Distance unavailable`로 렌더합니다 (#92). 단건 상세 API의 실제 화면 연결은 별도 후속입니다.
- **병원 (`facility:hira:<ykiho>`) → `501 NOT_IMPLEMENTED`.** HIRA 상세(`getDtlInfo2.8`)는 진료시간만 주고 이름·주소·좌표를 주지 않아 `ykiho`만으로는 시설을 복원할 수 없습니다. 마지막 segment는 base64url로 decode한 뒤 `ykiho`로 씁니다.
- **잘못된 형식·미지 provider·상류에 없는 hpid → `404 RESOURCE_NOT_FOUND`.**
