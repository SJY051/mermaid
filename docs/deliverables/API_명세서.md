...
- **`type=hospital`은 지금 `501 NOT_IMPLEMENTED`입니다.** 빈 배열을 돌려주면 "근처에 병원이 없다"는 거짓말이 됩니다. 공공 API는 승인됐고(§7), 구현(DEV-203)이 아직입니다.
...
**`GET /facilities?type=hospital`은 여전히 `501`입니다** — API는 열렸지만 구현(DEV-203)이 아직입니다.
막혀 있는 것과 안 만든 것은 다르고, 501은 후자를 뜻합니다.

구현되면 이름·주소·전화·좌표·거리·종별에 더해 **진료시간까지** 채울 수 있습니다.
다만 시간표가 없는 기관은 `operation.isOpenNow`가 `null`(모름)이어야 합니다 — `false`(닫힘)가 아니라 (spec §2-13).
