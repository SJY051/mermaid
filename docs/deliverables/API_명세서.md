...
- 항상 요청한 `limit` 이하의 가장 가까운 결과만 답합니다. 약국은 `open_now=true`일 때 위치 API가
  준 최대 100개 후보의 시간표를 조회한 뒤, 그중 가까운 열린 `limit`곳을 답합니다. 병원은 이 제한
  안의 후보만 상세 시간표를 조회하므로, `open_now=true`도 반경 전체에 대한 완전 검색은 아닙니다.
