...
- **약국 (`facility:nmc:<hpid>`) → `200`.** `getParmacyBassInfoInqire` 한 번으로 이름·주소·전화·좌표·주간 시간표를 전부 복원한 `Facility`를 돌려줍니다. 단, 단건 조회에는 기준 좌표가 없으므로 **`distanceMeters`는 `null`**입니다 (거짓 `0`이 아니라). `FE`는 이 필드를 `number | null`로 받아야 합니다.
