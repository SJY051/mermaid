...
> ./bin/verify-api-doc.sh                              # 32개 검사, 전부 통과해야 합니다
> ```
>
...
| `type` | enum | | `pharmacy` | `pharmacy` \| `hospital` |

- 항상 **거리순**으로 정렬됩니다. 정렬 파라미터는 없습니다.
- `open_now=true`는 **영업 중임을 아는** 곳만 남깁니다. 시간표를 읽지 못한 약국은 추측하지 않고 제외합니다.
- **`type=hospital`은 지금 `501 NOT_IMPLEMENTED`입니다.** 빈 배열을 돌려주면 "근처에 병원이 없다"는 거짓말이 됩니다 (DEV-203, 심평원 API 미승인).

**응답** `200` — `Facility[]`

```jsonc
[{
  "id": "facility:nmc:C1110693",     // 제공자 네임스페이스 포함. 이름·주소를 ID로 쓰지 않습니다
  "type": "pharmacy",
  "nameKo": "청실약국", "nameEn": null,
  "addressKo": "서울특별시 중구 …", "addressEn": null,
  "phone": "02-3789-6953",
  "lat": 37.5663, "lng": 126.9779,
  "distanceMeters": 140.0,           // 공공 API는 km로 줍니다. 여기서는 미터입니다
  "operation": {
    "isOpenNow": true,               // null 가능 = "모름". false는 "닫힘"이라는 단정입니다
    "status": "OPEN"|"CLOSED"|"UNKNOWN",
    "statusConfidence": "OFFICIAL_SCHEDULE"|"INFERRED",
...
`./bin/check-api-access.py`로 언제든 확인할 수 있습니다.

| 공공 API | 상태 |
|---|---|
| 약국 좌표 조회 | ✅ 200 |
| 의약품 허가정보 · e약은요 · DUR | ✅ 200 |
| 심평원 **병원정보서비스** (목록·`ykiho` 발급) | ✅ **200** (2026-07-10 승인) |
| 심평원 **의료기관별상세정보서비스** (진료시간) | ⛔ 403 — 활용신청 승인 대기 |
| 국립중앙의료원 병원·응급실 | ⛔ 403 — 신청 안 함 |

**병원 목록은 이제 부를 수 있고, 진료시간은 아직 못 부릅니다.**

그래서 `GET /facilities?type=hospital`이 구현되면 이렇게 됩니다.

- 이름·주소·전화·좌표·거리·종별 → **보여줄 수 있습니다**
- `operation.isOpenNow` → **`null` (모름)**. `false`(닫힘)로 단정하면 안 됩니다 (spec §2-13)
- `operation.status` → `UNKNOWN`, `statusConfidence` → `INFERRED`
- `open_now=true`로 거르면 **아무것도 남지 않습니다.** 영업 중임을 아는 병원이 하나도 없기 때문입니다

`./bin/check-api-access.py`가 이 표를 언제든 다시 확인해줍니다.
