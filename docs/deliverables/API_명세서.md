...
> `type=emergency_room`은 국립중앙의료원 위치 데이터입니다. 목록은 약 100m 격자 중심에서 조회·캐시하지만, 거리는 현재 요청 좌표에서 m로 다시 계산합니다. 운영시간은 제공되지 않아 `isOpenNow`가 항상 `null`이며 `open_now=true`에는 포함되지 않습니다. Fixture·hybrid fallback 결과는 `source.dataMode="fixture"`로 명시합니다.

### `GET /facilities/{id}` — 약국 `200` / 병원 `501`

UI-03 상세용. `id`는 네임스페이스 id입니다 (`facility:nmc:<hpid>` 또는 `facility:hira:<base64url(ykiho)>`).

> **HIRA id 인코딩 (이중 래핑).** HIRA의 `ykiho`는 원문이 `$481881#51#…` 같은 텍스트이고, HIRA가 이미 **표준 base64**(알파벳에 `+`,`/`,`=` 포함)로 인코딩해 내려줍니다. 이 문자열을 그대로 경로에 넣으면 `/`가 라우팅을 깨므로, 우리가 **다시 base64url(패딩 없음)로 한 번 더 래핑**해 마지막 segment로 씁니다 — 즉 `segment = base64url(HIRA가 준 base64 ykiho)`. 서버는 이 segment를 base64url로 decode해 ykiho를 복원합니다. base64url로 decode는 되지만 유효한 ykiho가 아닌 segment(예: `not-base64`)는 `501`이 아니라 **`404`**입니다. `bin/verify-api-doc.sh`의 `HIRA_FIXTURE_ID`가 이 계약을 실제로 검증합니다.
