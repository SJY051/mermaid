    ...
    <main className="mx-auto flex h-full max-w-2xl flex-col gap-4 p-6">
      ...
      {answer && (
        <section className="flex flex-col gap-3">
          ...
          {/* TODO(team): render the Naver map here — DEV-206. `uiActions` carries the query. */}
          {answer.uiActions.map((action, i) =>
            action.type === 'OPEN_FACILITY_MAP' ? (
              <p key={i} className="rounded-lg border border-border p-3 text-sm text-secondary">
                Map would show <strong>{action.payload.types.join(', ')}</strong> within{' '}
                {action.payload.radiusM}m{action.payload.openNow ? ', open now' : ''}
              </p>
            ) : null,
          )}
